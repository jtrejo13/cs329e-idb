...
class ItunesSongSpider(scrapy.Spider):
    ...
    def parse(self, response):
        song = response.css('h3 a::text').extract()[:10]
        artist = response.css('h4 a::text').extract()[:10]
        rank = response.css('li strong::text').extract()[:10]
        albumLink = response.css('li h3 a::attr(href)').extract()[:10]
